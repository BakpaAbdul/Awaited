...
function buildResultPayload(entry) {
  return {
    ...
    cycle_year: entry.cycleYear?.trim() || null,
    country: entry.country.trim(),
    study_level: entry.level?.trim() || null,
    field_of_study: entry.field?.trim() || null,
    host_university: entry.university?.trim() || null,
    program_name: entry.program?.trim() || null,
    application_round: entry.applicationRound?.trim() || null,
    status: entry.status,
    decision_date: entry.date,
    applied_date: entry.appliedDate || null,
    interview_date: entry.interviewDate || null,
    final_decision_date: entry.finalDecisionDate || null,
...
function mapResultRow(row, comments = []) {
  ...
  return {
    ...
    university: row.host_university || "",
