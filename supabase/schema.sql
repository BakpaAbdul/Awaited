...
create table if not exists public.scholarship_results (
  ...
  cycle_year text check (cycle_year is null or char_length(trim(cycle_year)) between 2 and 32),
  country text not null check (char_length(trim(country)) between 2 and 120),
  study_level text,
  field_of_study text check (field_of_study is null or char_length(trim(field_of_study)) between 2 and 160),
